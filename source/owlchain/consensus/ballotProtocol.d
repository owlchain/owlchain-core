...
public:
    ...
    ConsensusProtocol.EnvelopeState processEnvelope(ref Envelope envelope, bool self)
    {
        ...
        if (!isStatementSane(*statement, self))
        {
            if (self)
            {
                writefln("[ERROR], ConsensusProtocol not sane statement from self, skipping   e: %s",
        ...
        if (!isNewerStatement(*nodeID, *statement))
        {
            if (self)
            {
                writefln("[ERROR], ConsensusProtocol stale statement from self, skipping   e: %s",
                        mSlot.getCP().envToStr(envelope));
            }
            else
            {
                writefln("[TRACE], ConsensusProtocol stale statement, skipping   i: %d",
                        mSlot.getSlotIndex);
            }
            return ConsensusProtocol.EnvelopeState.INVALID;
        }
        ...
        if (validationRes != ConsensusProtocolDriver.ValidationLevel.kInvalidValue)
        {
            ...
            if (!processed)
            {
                ...
                else
                {
                    if (self)
                    {
                        writefln("[ERROR], ConsensusProtocol externalize statement with invalid value from self, skipping   e: %s",
                                mSlot.getCP().envToStr(envelope));
                    }
        ...
        else
        {
            ...
            if (self)
            {
                writefln("[ERROR], ConsensusProtocol invalid value from self, skipping   e: %s",
                        mSlot.getCP().envToStr(envelope));
            }
            else
            {
    ...
    void dumpInfo(ref JSONValue ret)
    {
        ...
        state.object["ballot"] = JSONValue(toUTF8(mSlot.getCP()
                .ballotToStr(cast(Ballot*) mCurrentBallot)));
    ...
    void dumpQuorumInfo(ref JSONValue ret, ref NodeID id, bool summary)
    {
        ...
        NodeID[] f = LocalNode.findClosestVBlocking(qSet, mLatestEnvelopes, (ref Statement st) {
        ...
        case StatementType.CP_ST_PREPARE:
            {
                ...
                {
                    mPrepared = cast(UniqueStruct!Ballot)(new Ballot(prep.prepared.counter,
                            prep.prepared.value));
                }
                if (prep.preparedPrime.counter > 0)
                {
                    mPreparedPrime = cast(UniqueStruct!Ballot)(new Ballot(prep.preparedPrime.counter,
...
private:
    ...
    void advanceSlot(ref Statement hint)
    {
        ...
        if (!mHeardFromQuorum && mCurrentBallot)
        {
            if (LocalNode.isQuorum(getLocalNode().getQuorumSet(),
                    mLatestEnvelopes, (ref Statement st) {
    ...
    bool attemptPreparedConfirmed(ref Statement hint)
    {
        ...
        for (idx = 0; idx < candidates.length; idx++)
        {
            ...
            bool ratified = federatedRatify((ref Statement st) {
                return BallotProtocol.hasPreparedBallot(*ballot, st);
            });
    ...
    bool attemptAcceptCommit(ref Statement hint)
    {
        ...
        IntervalPredicate pred = (ref Interval cur) {
            return federatedAccept((ref Statement st) {
                bool res = false;
                auto pl = &st.pledges;
                switch (pl.type)
                {
    ...
    bool attemptConfirmCommit(ref Statement hint)
    {
        ...
        IntervalPredicate pred = (ref Interval cur) {
            return federatedRatify((ref Statement st) {
                return BallotProtocol.commitPredicate(ballot, cur, st);
            });
        };
    ...
    bool setConfirmCommit(ref Ballot acceptCommitLow, ref Ballot acceptCommitHigh)
    {
        ...
        mCommit = cast(UniqueStruct!Ballot)(new Ballot(acceptCommitLow.counter,
                acceptCommitLow.value));
        mHighBallot = cast(UniqueStruct!Ballot)(new Ballot(acceptCommitHigh.counter,
        ...
        if (mPhase == CPPhase.CP_PHASE_PREPARE || mPhase == CPPhase.CP_PHASE_CONFIRM)
        {
            ...
            foreach (uint32 n; allCounters)
            {
                ...
                bool vBlocking = LocalNode.isVBlocking(getLocalNode()
                        .getQuorumSet(), mLatestEnvelopes, (ref Statement st) {
                            bool res;
                            auto pl = &st.pledges;
    ...
    bool setPrepared(ref Ballot ballot)
    {
        ...
        if (mPrepared)
        {
            ...
            if (comp < 0)
            {
                ...
                {
                    mPreparedPrime = cast(UniqueStruct!Ballot)(new Ballot(mPrepared.counter,
                            mPrepared.value));
                }
            ...
            else if (comp > 0)
            {
                ...
                {
                    mPreparedPrime = cast(UniqueStruct!Ballot)(new Ballot(ballot.counter,
                            ballot.value));
                    didWork = true;
                }
            }
    ...
    bool updateCurrentValue(ref Ballot ballot)
    {
        ...
        else
        {
            ...
            else if (comp > 0)
            {
                ...
                // with this situation
                writefln(
                        "[ERROR], ConsensusProtocol BallotProtocol.updateCurrentValue attempt to bump to a smaller value");
